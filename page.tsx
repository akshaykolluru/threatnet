...
type Detail = CaseItem & { evidence: Evidence[]; events: any[]; ranking: any[]; contradictions: any[]; audit: any[]; graph?: { nodes: GraphNode[]; edges: GraphEdge[] } };
...
function Overview({ detail, setView }: { detail: Detail; setView: (view: View) => void }) { return <><section className="metric-row"><Metric label="Evidence items" value={detail.evidence.length} /><Metric label="Timeline events" value={detail.events.length} /><Metric label="Review flags" value={detail.contradictions.length} /><Metric label="Audit entries" value={detail.audit.length} /></section><section className="dashboard-grid"><Panel title="Investigator relevance" action="Explainable signals"><div className="signal-list">{detail.ranking.map((item) => <div className="signal" key={item.id}><div><strong>{item.label}</strong><small>{item.reason}</small></div><b>{Math.round(item.score * 100)}%</b><span className="signal-bar"><i style={{ width: `${item.score * 100}%` }} /></span></div>)}</div></Panel><Panel title="Review flags" action="Needs attention"><div className="flag-list">{detail.contradictions.map((item) => <div className="flag" key={item.id}><span className="flag-dot" /><div><strong>{item.summary}</strong><small>{item.severity} severity · compare source records</small></div></div>)}</div></Panel></section><section className="dashboard-grid lower"><Panel title="Recent evidence" action="Source-linked records"><div className="evidence-list">{detail.evidence.map((item) => <div className="evidence-row" key={item.id}><span className="evidence-thumb">{item.type === "cctv" ? "O" : "[]"}</span><div><strong>{item.title}</strong><small>{item.notes}</small></div><span className="type-tag">{item.type}</span></div>)}</div></Panel><div className="quick-actions panel"><span className="eyebrow">Navigate</span><h3>See the story in context</h3><button onClick={() => setView("timeline")}>Open timeline <span>-&gt;</span></button><button onClick={() => setView("mindmap")}>Explore mind map <span>-&gt;</span></button><button onClick={() => setView("cctv")}>Drop in CCTV <span>-&gt;</span></button></div></section></>; }
...
function MindMap({ detail, hovered, setHovered, onSaved, onDelete }: { detail: Detail; hovered: Evidence | null; setHovered: (e: Evidence | null) => void; onSaved: () => Promise<void>; onDelete: (kind: "evidence" | "event", id: string) => Promise<boolean> }) {
  ...
  return <section className="mindmap-layout"><div className="panel mindmap-panel"><div className="panel-title"><div><span className="eyebrow">Relationship view / {detail.id}</span><h3>Evidence concept map</h3></div><span className="outline-chip">hover a label</span></div><ConceptMap detail={detail} setHovered={setHovered} />{detail.status === "Closed" && <div className="closed-notice">This case is closed. Reopen it before adding new records.</div>}<AddEvidenceForm caseId={detail.id} onSaved={onSaved} disabled={detail.status === "Closed"} /><EvidenceRecords items={detail.evidence} onDelete={onDelete} /><p className="graph-note">New evidence records are connected to this case automatically. Hover a label to inspect its source context.</p></div><aside className={`evidence-preview panel ${hovered ? "visible" : ""}`}><span className="eyebrow">Evidence preview</span>{hovered ? <><div className="preview-image">{previewImage ? <img src={previewImage} alt={hovered.title} /> : <><span>{hovered.type === "cctv" ? "CCTV" : "SOURCE"}</span><b>{hovered.id}</b></>}</div><h3>{hovered.title}</h3><p>{hovered.notes}</p><small>Source: {hovered.source}</small></> : <div className="preview-empty">Hover over an evidence label to inspect its source context.</div>}</aside></section>;
}
